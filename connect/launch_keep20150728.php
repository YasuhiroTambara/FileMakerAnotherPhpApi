...
function parse($XML){
  if(empty($XML)){
    exit('Error: Did not receive an XML document from the server.');
  }
...
function __construct($host = NULL, $file = NULL, $acc = NULL, $pas = NULL){
  $this->_props['host'] = (is_null($host) ? 'http://127.0.0.1:8080' : $host);
...
function _execute($_QUERYS){
  ...
  /*
  if($_OPTS = $this->_props['curlOpts']){
    foreach($_OPTS as $key => $v){
      curl_setopt($_CH, $key, $v);
    }
  }
  */
  //exit(iconv('ISO-8859-1', 'UTF-8', $this->_props['acc']));gzdeflate'㈱や①②'
  //exit(mb_convert_encoding('丹原','UTF-8','ISO-8859-1'));
  //exit( mb_detect_encoding($this->_props['acc']) );
  //exit( utf8_decode($this->_props['acc']) );
  //exit(utf8_encode('IrG6U+Rx0F5bLIQCUb9gOw=='));
  ...
  if($_CURLERR = curl_errno($_CH)){
    echo 'Communication Error: ('.$_CURLERR.') '.curl_error($_CH);
    if($_CURLERR == 52){
